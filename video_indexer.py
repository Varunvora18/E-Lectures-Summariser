...
def index_video(video_file):
    ...
    for key in list(key_text_set.keys()):
        ...
        for term in key_text_set[key]:
            if term not in key_terms and (term == term.title() or term == term.upper()) and all([x not in term for x in chars]):
                terms.append(term)
...
{title}\n\n"""
    ...
    with open("{}vtt".format(video_file[:-3]),"w") as myfile:
        myfile.write(text)

index_video(video_file)